...
if (isset($_POST['modificar'])) {
    ...
    if ($sql->query($updateQuery) === TRUE) {
        ...
        echo "Error actualizando el cliente: " . $sql->error;
    }
}
...
// Obtener datos del cliente a modificar si existe el ID en GET
if (isset($_SESSION["id"])) {
    $id = $_SESSION["id"];
    $registro = $sql->query("SELECT * FROM usuario WHERE id=$id");

    if ($registro && $registro->num_rows > 0) { // Verificar si hay resultados
        $organiza = $registro->fetch_assoc();
    } else {
        echo "No se encontraron datos para el usuario con ID: " . $id;
    }
}
...
<body data-spy="scroll" data-target=".navbar" data-offset="40" id="home">
    ...
    <div id="blog" class="container-fluid bg-dark text-light py-5 text-center wow fadeIn">
        <br><br>
        <h2 class="section-title py-5">Cambiar datos del cliente</h2>
        <div class="row justify-content-center">
            <div class="col-sm-7 col-md-4 mb-5">

            <form method="POST" action="modificar_clientes.php" enctype="multipart/form-data">
            <div class="form-group">
                    <label for="nombre" class="form-label">Cuenta del cliente: <?php echo htmlspecialchars($organiza['cuenta']); ?></label>
                    <input type="text" class="form-control" name="cuenta" value="<?php //echo htmlspecialchars($organiza['cuenta']); ?>">
                </div>
                <div class="form-group">
                    <label for="nombre" class="form-label">Nueva contraseña del cliente: <?php //echo htmlspecialchars($organiza['contrasena']); ?></label>
                    <input type="text" class="form-control" name="contrasena" value="<?php //echo htmlspecialchars($organiza['contrasena']); ?>">
                </div>
                <div class="form-group">
                    <label for="nombre" class="form-label">Nombre del cliente: <?php echo htmlspecialchars($organiza['nombre']); ?></label>
                    <input type="text" class="form-control" name="nombre" value="<?php //echo htmlspecialchars($organiza['nombre']); ?>">
                </div>
                <div class="form-group">
                    <label for="descripcion" class="form-label">Apellidos del cliente: <?php echo htmlspecialchars($organiza['apellidos']); ?></label>
                    <input type="text" class="form-control" name="apellidos" value="<?php //echo htmlspecialchars($organiza['apellidos']); ?>">
                </div>
                <div class="form-group">
                    <label for="precio" class="form-label">Email del cliente: <?php echo htmlspecialchars($organiza['email']); ?></label>
                    <input type="text" class="form-control" name="email" value="<?php //echo htmlspecialchars($organiza['precio']); ?>">
                </div>
                <div class="form-group">
                    <label for="imagen" class="form-label">Teléfono del cliente: <?php echo htmlspecialchars($organiza['telefono']); ?></label>
                    <input type="text" class="form-control" name="telefono" value="<?php //echo htmlspecialchars($organiza['imagen']); ?>">
                </div>
                <div class="form-group">
                    <label for="categoria" class="form-label">Ciudad del cliente: <?php echo htmlspecialchars($organiza['ciudad']); ?></label>
                    <input type="text" class="form-control" name="ciudad" value="<?php //echo htmlspecialchars($organiza['categoria']); ?>">
                </div>
                <div class="form-group">
                    <label for="alergenos" class="form-label">Código postal del cliente: <?php echo htmlspecialchars($organiza['cp']); ?></label>
                    <input type="text" class="form-control" name="cp" value="<?php //echo htmlspecialchars($organiza['alergenos']); ?>">
                </div>
                <div class="form-group">
                    <label for="proveedor" class="form-label">Calle del cliente: <?php echo htmlspecialchars($organiza['calle']); ?></label>
                    <input type="text" class="form-control" name="calle" value="<?php //echo htmlspecialchars($organiza['calle']); ?>">
                </div>
                <div class="form-group">
                    <label for="tiempoPreparacion" class="form-label">Número calle del cliente: <?php echo htmlspecialchars($organiza['numero']); ?></label>
                    <input type="text" class="form-control" name="numero" value="<?php //echo htmlspecialchars($organiza['numero']); ?>">
                </div>
                <div class="form-group">
                    <label for="tiempoPreparacion" class="form-label">Piso del cliente: <?php echo htmlspecialchars($organiza['piso']); ?></label>
                    <input type="text" class="form-control" name="piso" value="<?php //echo htmlspecialchars($organiza['numero']); ?>">
                </div>
                <div class="form-group">
                    <label for="tiempoPreparacion" class="form-label">Letra del cliente: <?php echo htmlspecialchars($organiza['letra']); ?></label>
                ...
                <div class="form-group">
                    <button type="submit" name="modificar" class="form-control btn btn-primary">Modificar datos del cliente</button><br><br>
                    <a href="ver_clientes.php?id=<?php echo $organiza["id"]; ?>" class="btn btn-outline-secondary">Volver</a>
                </div>
            </form>
            </div>
        </div>
